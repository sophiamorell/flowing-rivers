...
| #  | Item | Status | Where in the code |
|----|------|--------|-------------------|
| 15 | **Testimonials (E-24) not built.** Needs, before anything goes on the page: written permission from all six people; parental consent for Ayla (7) and Mason (5), ideally shown as age only; a decision on Stephanie and Carrie, who describe a peer-coach relationship rather than a client one (label accurately or leave out); Kellie's two HRV sentences removed (a measurable health claim); and confirmation of whether "Sophie, 39" is you, which would need disclosure. | **Blocking E-24** | `app/page.tsx` (marker where the section goes) |
...
| 26 | **End-to-end email test** with a real Resend key, once items 6 and 24 are set in Netlify. | Before launch | `app/api/contact/route.ts` |
